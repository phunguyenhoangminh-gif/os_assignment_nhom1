...
\begin{itemize}
    ...
    \item \textbf{Đảm bảo độ phản hồi:} Bằng cách luôn chọn tiến trình từ hàng đợi có độ ưu tiên cao nhất (nếu có), hệ thống đảm bảo các tác vụ quan trọng được xử lý ngay lập tức.
...
\textbf{Giai đoạn 1: Lựa chọn dựa trên vị trí}
\begin{enumerate}
    ...
    \item Nếu thỏa mãn: dequeue process, tăng \texttt{current\_slot[prio]}, cập nhật \texttt{current\_prio}.
    \item Dừng ngay khi tìm thấy process phù hợp.
\end{enumerate}

\textbf{Giai đoạn 2: Tái lập \& Thử lại}

Nếu Giai đoạn 1 không tìm thấy process (tất cả các vị trí đã hết):
\begin{enumerate}
    \item Tái thiết lập tất cả \texttt{current\_slot[]} về 0.
...
\begin{itemize}
    \item \textbf{Priority preemption:} Mỗi lần chọn process, thuật toán duyệt từ priority 0 (highest) đến priority 139 (lowest), đảm bảo processes có mức độ ưu tiên cao hơn luôn được xử lý trước.
    \item \textbf{Slot fairness:} Giai đoạn 2 được kích hoạt khi tất cả queues có processes đã hết vị trí, reset bộ đếm và bắt đầu round mới, ngăn đói tài nguyên (starvation) cho các processes có mức ưu tiên thấp hơn.
    \item \textbf{Time complexity:} O(MAX\_PRIO) = O(140) = O(1) thời gian là một hằng số cho mỗi lần get process.
