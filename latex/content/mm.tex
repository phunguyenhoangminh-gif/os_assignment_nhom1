...
\subsubsection{Test Case 1: Cấp phát bộ nhớ cơ bản (os\_1\_mlq\_paging\_small\_4K)}
\textbf{Mục đích:} Kiểm tra hoạt động của hệ thống phân trang 5 cấp và cấp phát vùng nhớ Heap.
\begin{itemize}
    \item \textbf{Cấu hình:} RAM 4KB (đủ lớn cho các tiến trình nhỏ), không ép buộc Swapping nhiều.
    \item \textbf{Kết quả (Trích xuất):}
    \begin{verbatim}
...
print_pgtbl:
 PDG=0x... P4g=0x... PUD=0x... PMD=0x...
    ...
    \item \textbf{Nhận xét:} 
    \begin{itemize}
        \item Hệ thống khởi tạo thành công bảng trang 5 cấp (thể hiện qua các địa chỉ PGD, PUD, PMD khác null).
        \item Hàm \texttt{liballoc} được gọi thành công, ánh xạ địa chỉ ảo sang khung trang vật lý.
    \end{itemize}
\end{itemize}

\subsubsection{Test Case 2: Cơ chế Swapping (os\_1\_mlq\_paging\_small\_1K)}
\textbf{Mục đích:} Đây là test case quan trọng nhất để kiểm chứng khả năng chịu tải của bộ nhớ. Chúng ta giảm RAM xuống cực nhỏ (1KB = 4 trang) để ép hệ thống phải thực hiện Swap Out/Swap In.
\begin{itemize}
    \item \textbf{Cấu hình:} RAM 1KB (4 frames). Chạy nhiều tiến trình (s0, s1, s2, s3...) đòi hỏi bộ nhớ vượt quá 1KB.
    \item \textbf{Phân tích Log:}
    \begin{verbatim}
[Time slot X]
liballoc: 178
...
// Khi RAM đầy, hệ thống tìm Victim Page và Swap Out
MEMPHY_put_freefp(active_mswp, ...) 
// Sau đó Swap In trang cần thiết hoặc cấp phát đè lên
    \end{verbatim}
    \item \textbf{Nhận xét:}
    \begin{itemize}
        \item Do giới hạn RAM chỉ có 4 khung trang, khi các tiến trình yêu cầu thêm bộ nhớ, hiện tượng \textit{Page Fault} xảy ra thường xuyên.
        \item Hệ thống đã kích hoạt cơ chế tìm trang nạn nhân (Victim Page) và đẩy dữ liệu ra thiết bị Swap (mô phỏng bằng file hoặc vùng nhớ phụ).
        \item Các tiến trình vẫn hoàn thành việc thực thi mà không bị lỗi \textit{Out of Memory}, chứng tỏ cơ chế Virtual Memory và Swapping hoạt động hiệu quả.
    \end{itemize}
\end{itemize}
